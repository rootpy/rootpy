...
class DataManager(object):
    ...
    def get_object_by_name(self, name):
        ...
        if self.__use_rootfs:
            for file, filename in [(self.pluggedData, self.pluggedDataName),
                                   (self.coreData, self.coreDataName)]:
                if file:
    ...
    def get_tree(self, treepath, maxEntries=-1, fraction=-1, cuts=None):
        ...
        if cuts == None:
            cuts = Cut("")
        if self.verbose:
            print "Fetching tree %s..." % treepath
        tmpTree = None
        tree = self.get_object_by_name(treepath)[0]
        if not tree:
            if self.verbose:
                print "Tree %s not found!" % treepath
            return None
        friends = tree.GetListOfFriends()
        if friends:
            if len(friends) > 0 and self.verbose:
                print("Warning! ROOT does not play nice with friends where "
                      "cuts are involved!")
            if len(friends) == 1:
                if self.verbose:
                    print("Since this tree has one friend, I will assume that "
                          "it's friend is the core data (read-only) "
                          "and you want that tree instead")
                    print "where cuts may refer to branches in this tree"
                tmpTree = tree
                friendTreeName = friends[0].GetTreeName()
                if friends[0].GetTitle() in self.friendFiles:
        ...
        if fraction > -1.:
            ...
            if self.verbose:
                print("Extracting %.1f%% of the tree which contains %i "
        ...
        if finalNumEntries > 0 and originalNumEntries != finalNumEntries:
            tree.SetWeight(tree.GetWeight() * float(originalNumEntries) /
                           float(finalNumEntries))
        if self.verbose:
            print("Found %s with %i entries and weight %e" %
                  (treepath, tree.GetEntries(), tree.GetWeight()))
        if cuts:
    ...
    def get_sample(self, samplestring, treetype=None, cuts=None, maxEntries=-1,
                   fraction=-1, properties=None):
                ...
                for branch in self.trees[sampletype]:
                    if tree.GetBranch(branch):
                        if branch in self.variables:
                            if 'alias' in self.variables[branch]:
