...
class List(Object, QROOT.TList):
    ...
    def Add(self, value, *optional):
        """
        Overload ROOT's basic TList::Add to support supplying TListItemWithOption
        """
        
        if isinstance(value, TListItemWithOption):
            if optional:
                raise RuntimeError("option specified along with "
                    "TListItemWithOption. Specify one or the other but not both.")
    ...
    def __setitem__(self, idx, desired):
        ...
        if not isinstance(desired, (list, tuple)):
            raise NotImplementedError("Only support list or tuple in slice assignment")
    ...
    def __getitem__(self, idx):
        """
        Similar to list(self)[idx] except it uses ``List.as_list_with_options``.
        """
